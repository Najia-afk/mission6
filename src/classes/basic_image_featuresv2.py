...
- Normalization of images by product category
...
class BasicImageFeatureExtractor:
    ...
    def normalize_images_by_category(self, df, processed_images, num_images_per_category=None, random_seed=None):
        """
        Normalize images by product category and compute standardized features.

        Args:
            df (pd.DataFrame): DataFrame containing image metadata, including 'product_category' and 'uniq_id'.
            processed_images (list): List of processed images corresponding to the 'uniq_id' in the DataFrame.
            num_images_per_category (int, optional): Number of images to sample per category. If None, use all images.
            random_seed (int, optional): Random seed for reproducibility when sampling images.

        Returns:
            dict: A dictionary where keys are product categories and values are normalized images.
        """
        ...
        for category in categories:
            ...
            category_images = df[df['product_category'] == category]
            category_image_ids = category_images['uniq_id'].tolist()

            # Match images by uniq_id
            category_processed_images = [
                img for img, img_id in zip(processed_images, df['uniq_id']) if img_id in category_image_ids
            ]

            # Sample a subset of images if num_images_per_category is specified
            if num_images_per_category is not None and len(category_processed_images) > num_images_per_category:
                sampled_indices = np.random.choice(len(category_processed_images), num_images_per_category, replace=False)
                category_processed_images = [category_processed_images[i] for i in sampled_indices]
                category_image_ids = [category_image_ids[i] for i in sampled_indices]

            # Extract features for the category
            feature_results = self.extract_features_batch(
                category_processed_images,
                image_names=category_image_ids
            )

            # Compute mean and standard deviation for each feature type
            normalized_features = {
                'sift': np.mean(feature_results['sift_features'], axis=0) if len(feature_results['sift_features']) > 0 else None,
                'lbp': np.mean(feature_results['lbp_features'], axis=0) if len(feature_results['lbp_features']) > 0 else None,
                'glcm': np.mean(feature_results['glcm_features'], axis=0) if len(feature_results['glcm_features']) > 0 else None,
                'gabor': np.mean(feature_results['gabor_features'], axis=0) if len(feature_results['gabor_features']) > 0 else None,
                'patch': np.mean(feature_results['patch_features'], axis=0) if len(feature_results['patch_features']) > 0 else None,
            }

            # Store the normalized features as a "normalized image"
            normalized_images[category] = normalized_features
    ...
    def create_feature_visualization(self):
        """
        Create a comprehensive visualization of the extracted features.

        Returns:
            plotly.graph_objects.Figure: A Plotly figure object for visualization.
        """
        # Ensure features are extracted
        if self.combined_features is None or self.feature_names is None:
            raise ValueError("No features available for visualization. Extract features first.")

        # Create a subplot for each feature type
        fig = make_subplots(
            rows=2, cols=3,
            subplot_titles=("SIFT Features", "LBP Features", "GLCM Features", "Gabor Features", "Patch Features"),
            specs=[[{"type": "scatter"}, {"type": "scatter"}, {"type": "scatter"}],
                [{"type": "scatter"}, {"type": "scatter"}, None]]
        )

        # Add SIFT feature visualization
        if len(self.feature_results['sift_features']) > 0:
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(self.feature_results['sift_features'][0]))),
                    y=np.mean(self.feature_results['sift_features'], axis=0),
                    mode='lines',
                    name="SIFT Features"
                ),
                row=1, col=1
        ...
        # Add LBP feature visualization
        if len(self.feature_results['lbp_features']) > 0:
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(self.feature_results['lbp_features'][0]))),
                    y=np.mean(self.feature_results['lbp_features'], axis=0),
                    mode='lines',
                    name="LBP Features"
                ),
                row=1, col=2
        ...
        # Add GLCM feature visualization
        if len(self.feature_results['glcm_features']) > 0:
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(self.feature_results['glcm_features'][0]))),
                    y=np.mean(self.feature_results['glcm_features'], axis=0),
                    mode='lines',
                    name="GLCM Features"
                ),
                row=1, col=3
        ...
        # Add Gabor feature visualization
        if len(self.feature_results['gabor_features']) > 0:
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(self.feature_results['gabor_features'][0]))),
                    y=np.mean(self.feature_results['gabor_features'], axis=0),
                    mode='lines',
                    name="Gabor Features"
                ),
                row=2, col=1
        ...
        # Add Patch feature visualization
        if len(self.feature_results['patch_features']) > 0:
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(self.feature_results['patch_features'][0]))),
                    y=np.mean(self.feature_results['patch_features'], axis=0),
                    mode='lines',
                    name="Patch Features"
                ),
                row=2, col=2
        ...
        # Update layout
        fig.update_layout(
            title="Comprehensive Feature Visualization",
            height=800,
            width=1200,
            showlegend=True
        )
